...
double hiddenScore(State nowState) {
  ...
  for(int i=0; i<HEIGHT; i++) for(int j=0; j<WIDTH; j++) if(nowState.field[i][j]==EMPTY || nowState.flashPoint == make_pair(i,j)) {
	vector<int> cand;
	for(int d=0; d<6; d++) {
	  int sum = 0;
	  int nx = j;
	...
	if(nowState.field[i][j]!=EMPTY) {
	  if(nowState.field[i][j]>SUM )d /= 100.0;
	  else d *= (111.0-(double)nowState.turn/10.0);
	}
	if(nowState.field[i+1][j]!=EMPTY) {
	  if(nowState.field[i+1][j]>SUM)  d /= 100.0;
	  else d *= 2.0;
	}
	

	//	for(int k=i+2; k<=HEIGHT; k++) if(nowState.field[k][j]!=EMPTY) d/=2.0;
	for(int k=i-1; k>=0; k--) if(nowState.field[k][j]==EMPTY) d/=1.4;
	int leftHeight = HEIGHT*2 ;
	int rightHeight = HEIGHT*2;
	if(j!=0)       for(leftHeight=0; nowState.field[leftHeight][j-1]!=EMPTY;leftHeight++);
	if(j!=WIDTH-1) for(rightHeight=0; nowState.field[rightHeight][j+1]!=EMPTY;rightHeight++);

	for(int k=0; k<abs(leftHeight-i)+abs(rightHeight-i)-2; k++) d/=1.4;
	...
	for(int k=0; k<csize; k++) {
	  ...
	  for(int x=0; x<cntsize; x++) score *= (3.0+(double)tmpState.turn/100);
	  for(int x=0; x+1<cntsize; x++) {
	    int BORDER  = 2;
	  ...
	  for(int z=0; z<i; z++) score *= 1.0001;
      	  //for(int z=tmpState.turn/40+1;z<i; z++ ) 
	  for(int z=cntsize/4+4; z<i; z++)
	    score /= 1.1;
      	  //if(tmpState.turn<400) 
      	  if(cntsize<20) 
	    //  for(int y=tmpState.turn/40+2; y<HEIGHT; y++) 
	    for(int y=cntsize/4+4; y<HEIGHT; y++) 
	      for(int x=0; x<WIDTH; x++)
		if(tmpState.field[y][x]<=SUM)
		  score /= 7.0;
	  
	  
	  if(score>bestscore) {
	    bestscore = score;
	    nowState.flashPoint = make_pair(i,j) ;
	  }
...
void running() {
  ...
  for(nowState.turn = 0; nowState.turn<STEP; nowState.turn++){
    ...
    for(int xpos = -SIZE+1; xpos<WIDTH -1; xpos++) {
	...
	if(!isGameover) {
	  ...
	  if(score>bestscore) {
	    ...
	    bestRot = rot; 
	    bestscore = score;
	    nowState.flashPoint = tmpState.flashPoint;
	  }
	}
