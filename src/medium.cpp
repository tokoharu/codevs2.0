...
double hiddenScore(State nowState) {
  ...
  for(int i=0; i<HEIGHT; i++) for(int j=0; j<WIDTH; j++) if(nowState.field[i][j]==EMPTY) {
	vector<int> cand;
	for(int d=0; d<6; d++) {
	  int sum = 0;
	  int nx = j;
	...
	for(int k=0; k<abs(leftHeight-i) + abs(rightHeight-i) ; k++) d /= 1.2;
	...
	for(int k=0; k<csize; k++) {
	  ...
	  for(int x=0; x<cntsize; x++) score *= 8.0;
	  for(int x=0; x<cntsize-3; x++) {
	    int BORDER  = 2;
	  ...
	  for(int z=cntsize/3+2; z<i; z++) score /= 1.1;
	  if(cntsize<20)
	  for(int y=cntsize/3+2; y<HEIGHT; y++) 
	    for(int x=0; x<WIDTH; x++)
